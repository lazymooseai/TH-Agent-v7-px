...
def get_averio_ships():
    ...
    try:
        ...
        # Etsitään kaikki taulukon rivit turvallisesti
        for taulu in soup.find_all("table"):
            for tr in taulu.find_all("tr"):
                tds = [td.get_text(strip=True) for td in tr.find_all(["td", "th"])]
                
                # TURVALUKKO: Jos rivillä ei ole tarpeeksi sarakkeita (esim. tyhjä rivi), ohitetaan se
                ...
                # Poimitaan matkustajien määrä numeerisena
                pax = None
